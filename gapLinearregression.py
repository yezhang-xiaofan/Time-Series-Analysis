#This file use gap linear regression
import numpy as np
from Mocapy import *
from numpy import *
from scipy import *
...
GapList = list()
MseList = list()
AverageErrorList = list()
for Gap in range(3,19):
    GapList.append(Gap)
### Set the length of the gap
#Gap = 3
    #Create linear regression object
    NumAttribute = max([len(WaitingTimeSeq[key]) for key in WaitingTimeSeq]) - Gap
    OutputList = list()
    InputList = list()
    for key in range(len(WaitingTimeSeq)):
        if(len(WaitingTimeSeq[key])>Gap): 
            for i in range(Gap,len(WaitingTimeSeq[key])):
                #tempOutput = centroids[WaitingTimeSeq[key][i]]
                tempOutput = WaitingTimeSeq[key][i]
                OutputList.append(tempOutput)
                tempList = WaitingTimeSeq[key][:(i-Gap+1)][:]
                #tempList = [centroids[j] for j in tempList]        
                tempList.reverse()
                tempList += [0]*(NumAttribute-len(WaitingTimeSeq[key][:(i-Gap+1)]))
    ...
    for key in range(len(TestWaitTimeSeq)):
        if(len(TestWaitTimeSeq[key])>Gap):
            for i in range(Gap,len(TestWaitTimeSeq[key])):
                TrueValueList.append(centroids[TestWaitTimeSeq[key][i]])
              #TestInputList.append(TestWaitTimeSeq[key][:i])
                tempList = TestWaitTimeSeq[key][:(i-Gap+1)][:]
                tempList.reverse()
                tempList += [0]*(NumAttribute-len(TestWaitTimeSeq[key][:i-Gap+1]))
                tempArray = np.array(tempList)
    ...
    #Calculate the mse
    Residual = np.array(PredictArray) - np.array(TrueValueArray)
...
GapList = [i*10 for i in GapList]
fig1 = plt.figure()
#plt.plot(GapList,MseList)
plt.plot(GapList,AverageErrorList,linewidth=4)

plt.xlabel('Time gap (minutes)',fontsize=19)
plt.ylabel('Average error (hours)',fontsize=19)
plt.savefig('gapLinearregression.eps')
plt.show()
