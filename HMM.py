#This file considers "Weekday + time of day"
...
TimeString = [str(x).zfill(4) for x in TimeList]
TimeString = [(str(int(x[0])+1) + x[1:]) for x in TimeString]
TimeValueString = [str(x).zfill(4) for x in list(TimeValue)]
TimeValueString = [(str(int(x[0])+1) + x[1:]) for x in TimeValueString]
PredictWaiting = DataFrame(zeros(NumTime,NumWaitingTime),index=TimeValueString,columns=WaitingTimeValue)
for i in range(len(WaitTimeList)):
    tempindex = TimeString[i]
    tempwaitingtime = WaitTimeList[i]
    count = PredictWaiting[tempindex][i]
    count = count + 1
    PredictWaiting[tempindex][i] = count

for row in TimeValueString:
    PredictWaiting[row] = PredictWaiting[row]/sum(PredictWaiting[row])
...
# wt,h = argmax(wt) p(wt,h|wt-1) = argmax(wt) p(wt-1|wt)*(wt,h)
BaseLinePre = Series(zeros(NumTime),index=TimeValueString)
