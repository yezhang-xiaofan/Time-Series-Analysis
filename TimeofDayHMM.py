...
for i in range(len(TrainWaitTimeArray)):
    Hour = TrainTimeList[i][1] + TrainTimeList[i][2] + TrainTimeList[i][3]
...
for i in range(len(WaitTimeArray)): 
    ...
    DisWaitingTime[i] = temp      ###DiswaitingTime stores the state number 
       
#Construct the training data, WaitTimeList and TimeList is training data
TimeList = list()
for i in xrange(len(WaitTimeArray)/2):
    TimeList.append(AllTimeList[i])
    WaitTimeList.append(DisWaitingTime[i])    #WaitTimeList stores the state number of waiting time
...
for key in WaitingTimeSeq:
    ...
    for i in xrange(0,len(WaitingTimeSeq[key])-1):
...
##TestTimeSeq stores time sequence values in string format ['151','152'...]
##TestWaitTimeSeq stores waiting time values in state number [0,1,2,...]
totalError = 0.0
totalCount = 0.0
for key in TestWaitTimeSeq:
    observations = map(int,TestTimeSeq[key])
    observations = map(list(TimeValue).index,observations)
    Prediction = emitter.predict(observations)
    ContinousPrediction = [centroids[x] for x in Prediction]
    TrueValue = [centroids[x] for x in TestWaitTimeSeq[key]]
    errors = sum([a-b for a,b in zip(ContinousPrediction,TrueValue)])
    totalError += errors
    totalCount += len(TestWaitTimeSeq[key])
averageError = totalError/totalCount
print "Average Error is: " + str(averageError) 
##########The average error is 0.2981 hour
