#This file uses polynomial regression
import numpy as np
from Mocapy import *
from numpy import *
from scipy import *
...
for key in range(len(WaitingTimeSeq)):
    for i in range(1,len(WaitingTimeSeq[key])):
        ...
        tempList = list()
        for j in range(i):            
            tempList.append(WaitingTimeSeq[key][j])
            tempList.append(WaitingTimeSeq[key][j]**2)
        tempList.reverse()
        tempList += [0]*2*(NumAttribute-len(WaitingTimeSeq[key][:i]))
...
#beta = mf.multipolyfit(InputMatrix,OutputMatrix,2)
beta = linalg.lstsq(InputMatrix,OutputMatrix)[0]
...
for key in range(len(TestWaitTimeSeq)):
    for i in range(1,len(TestWaitTimeSeq[key])):
        ...
        tempList = list()
        for j in range(i):
            tempList.append(TestWaitTimeSeq[key][j])
            tempList.append(TestWaitTimeSeq[key][j]**2)        
        tempList.reverse()
        tempList += [0]*2*(NumAttribute-len(TestWaitTimeSeq[key][:i]))
        tempArray = np.array(tempList)
...
#TestInputMatrix = hstack((ones((TestInputMatrix.shape[0], 1), dtype=TestInputMatrix.dtype) , TestInputMatrix))

#generators = [basis_vector(NumAttribute+1, i) for i in range(NumAttribute+1)]

    # All combinations of degrees
#deg = 2

#temp = itertools.combinations_with_replacement(generators, deg)
#results = [x for x in temp]
#results = [x[0]+x[1] for x in results]
#powers = results

    # Raise data to specified degree pattern, stack in order
#A = hstack(asarray([as_tall((TestInputMatrix**p).prod(1)) for p in powers]))
#PredictArray = np.dot(TestInputMatrix,rlm_results.params)
PredictArray = np.dot(TestInputMatrix,beta)
