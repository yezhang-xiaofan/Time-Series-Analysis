#This file buids HMM using only "hour" as one bucket.
import numpy as np
from Mocapy import *
from numpy import *
from scipy import *
...
for i in range(len(TrainWaitTimeArray)):
    ######################################Considering only hour
    Hour = TrainTimeList[i][1] + TrainTimeList[i][2]
...
for key in sorted(HourMap.iterkeys()):
    HourKeyList.append(key)
HourValue = sorted(HourMap.iterkeys())   
...
w= csv.writer(open("GaussianMap.csv","w"))
for key, val in GaussianMap.items():
    w.writerow([key,val])
...
#convert the time value to integer
HourList = [(x[1]+x[2]) for x in TimeList]
...
#Construct the sequence with the index "hour"
key = 0
while(index<len(HourList)):
    WaitingTimeSeq[key] = list()
    HourSeq[key] = list()
    OriginalWaitingTimeSeq[key] = list()
    print key
    while (True):    
        if((index+2)<=len(HourList) and HourList[index+1] >= HourList[index]):
            WaitingTimeSeq[key].append(DisWaitingTime[index])
            HourSeq[key].append(HourList[index])
            OriginalWaitingTimeSeq[key].append(WaitTimeArray[index])
            index = index + 1
            print index
            continue
...
for key in WaitingTimeSeq:
    ...
    for i in range(len(WaitingTimeSeq[key])):
        #pad string !!!
        OriKey = str(TimeSeq[key][i]).zfill(4)
        HourKey = OriKey[1] + OriKey[2]
        if(HourSeq[key].has_key(HourKey)):
            HourSeq[key][HourKey].append(WaitingTimeSeq[key][i])
        else:
            HourSeq[key][HourKey] = list()
            HourSeq[key][HourKey].append(WaitingTimeSeq[key][i])
'''
...
#Construct the transition matrix to compute P(wt-1|wt,h)
NewTransition = Panel(np.zeros(NumHour,NumWaitingTime,NumWaitingTime), items=HourValue
                      ,major_axis=WaitingTimeValue, minor_axis=WaitingTimeValue)
for key in HourSeq:
    for innerkey in HourSeq[key]:
        code
