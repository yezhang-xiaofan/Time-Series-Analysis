# This file draws plots for offices considering weekday
#This file draw error bar for each hour 
import numpy as np
from Mocapy import *
from numpy import *
from scipy import *
...
####################Fit a Gaussian model for each 10 min bucket without considering weekday 
...
##############################################Change the weekday here
weekday = 4
bucketList = filter(lambda x: int(x[0])==weekday,bucketList)
...
ind = np.arange(0.5,len(bucketList)+0.5,1)
width = 1
matplotlib.rc('xtick',labelsize=19)
matplotlib.rc('ytick',labelsize=19)
text_file = open(str(Officeid)+"data","w")
for i in range(len(meanArray)):
    text_file.write(repr(meanArray[i])+","+repr(widthArray[i])+"\n")

text_file.close()
fig = plt.figure()
ax = fig.add_subplot(111)
#ax.bar(ind,meanArray,width,color='r',yerr=widthArray)
ax.errorbar(ind,meanArray,yerr=widthArray,elinewidth=2,linewidth=4)
#hours = mdates.HourLocator()
Ind = np.arange(0.5,54,24)
ax.set_xticks(Ind)
ax.set_xticklabels(['8:00','12:00','16:00'])
...
#fig.autofmt_xdate()
ax.set_ylabel('Wait time (hours)',fontsize=19)
ax.set_xlabel('Time of day',fontsize=19)
#ax.set_title('Waiting time by time bucket (10 minutes) for office '+str(Officeid))
plt.gca().yaxis.set_major_locator(MaxNLocator(nbins=6,prune='lower'))
#fig.autofmt_xdate()
plt.savefig('Office'+str(Officeid)+'weekday'+str(weekday+1)+'.eps',format='eps')
plt.show()
